MainModule
    .controller("InvLedgerIndexCtlr", function ($scope,$window, $http) {
        ...
        init_Operations($scope, $http,
            '/Inventory/Proceedings/InvLedgerLoad', //--v_Load
            '', // getrow
            '' // PostRow
        );

        init_ViewSetup($scope, $http, '/Inventory/Proceedings/GetInitializedInvLedger');

        $scope.init_ViewSetup_Response = function (data) {
            if (data.find(o => o.Controller === 'InvLedgerIndexCtlr') != undefined) {
                $scope.Privilege = data.find(o => o.Controller === 'InvLedgerIndexCtlr').Privilege;
                init_Filter($scope, data.find(o => o.Controller === 'InvLedgerIndexCtlr').WildCard, null, null, null);    
                init_Report($scope, data.find(o => o.Controller === 'InvLedgerIndexCtlr').Reports, '/Inventory/Proceedings/GetInvLedgerReport'); 
        ...
        $scope.OpenReport = function (name) {
            if (name === 'Ledger' && $scope.ParaProdID > 0)
                $window.open('/Inventory/Proceedings/GetInvLedgerReport?rn=Ledger&id=' + $scope.ParaProdID + "&datefrom=" + new Date($scope.ParaDateFrom).toLocaleString('en-US') + "&datetill=" + new Date($scope.ParaDateTill).toLocaleString('en-US'));
            else if (name === 'Stock')
                $window.open('/Inventory/Proceedings/GetInvLedgerReport?rn=Stock&id=' + $scope.ParaWareHouseID + "&datefrom=" + new Date($scope.ParaDateFrom).toLocaleString('en-US') + "&datetill=" + new Date($scope.ParaDateTill).toLocaleString('en-US'));
        };
